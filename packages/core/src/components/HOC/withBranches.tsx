...
(ShapeComponent:React.FC< baseComponentAttrs & {node:NodeRef,parent?:NodeRef,curvature?:number}>)
    : React.FC<BranchProps<baseComponentAttrs>>  {
    
        const withNodes = (props:BranchProps<baseComponentAttrs>)=>{
        
        const {branches,keyBy=(n:NodeRef)=>n._id,attrs={}, curvature, interactions={}} = props
    ...
    `withBranchArray(${ShapeComponent.displayName || ShapeComponent.name || 'Component'})`;
    return withNodes
