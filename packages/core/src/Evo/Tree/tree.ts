...
export class Tree implements TreeInterface {
    ...
    getInternalNodeCount(): number {
            return this.nodeList.filter(node=>node.children.length>0).length;
        }
    ...
    getChildCount(node: NodeRef): number {
            return (node as Node).children.length;
        }
    ...
    getNodeByLabel(label: string): NodeRef | undefined {
            return this.getNodeByLabel(label);
        }
    getLevel(node: NodeRef): number {
            return (node as Node).level;
        }
    ...
    getParent(node: NodeRef): NodeRef | undefined {
            return (node as Node).parent;
        }
    getChildren(node: NodeRef): NodeRef[] {
            return (node as Node).children;
        }
     ...
     * @returns {object}
     */
    getSiblings(node:Node):Node[]|null {
        if (!node.parent) {
            return null;
        }
    ...
    *postorder(startNode=this.root,filter=()=>true) {
     ...
     * @returns {IterableIterator<IterableIterator<*|*>>}
     */
    static* pathToRoot(node:Node|undefined):Generator<Node> {
        while (node) {
            yield node;
            node = node.parent;
        }
     ...
     * @returns {number}
     */
    pathLength(node1:Node, node2:Node) {
    ...
    //         .reduce((acc:Node[],curr)=> { // reduce to the unique set.
     ...
     * @param splits
     */
    // splitBranches(splits:Node[]|null = null) {
     ...
     */
    annotateTips(annotations:{[key:string]:any}) {
     ...
     */
    annotateNodes(annotations:{[key:string]:any}) {
        for (let [key, values] of Object.entries(annotations)) {
    ...
    addAnnotations(annotations: { } ) {
        for (let [key, addValues] of Object.entries(annotations)) {
            let annotation:any = this.annotations[key];
            if (!annotation) {
                annotation = {};
            ...
            if (Array.isArray(addValues)) {
                ...
                let sum = 0.0;
                let keys:string[] = [];
                for (let [key, value] of Object.entries(addValues as Object)) {
                ...
                if (typeof addValues === typeof true) {
                    type = Type.BOOLEAN;
                } else if (!isNaN(addValues as number)) {
                    type = (addValues as number % 1 === 0 ? Type.INTEGER : Type.FLOAT);
                }
    ...
    getClades(tipNameMap:Map<Node,number>){
            ...
            if(inAnnotation){
                ...
                if (lengthNext) {
                    ...
                    if (!currentNode!.label.startsWith("#")) {
                        let value:number|string = parseFloat(currentNode!.label);
                        if (isNaN(value)) {
                            value = currentNode!.label;
                        }
                        let label_annotation:{[key:string]:any}={};
                    ...
                    const externalNode = tree.newNode( {name: name.replace(/\'/g,'')},
                        true);
                    if(decimalDate!==null){
     ...
     */
   static  parseNexus(nexus:string,options={}){
 ...
 * @returns {*}
 */
//TODO check discrete annotation
function fitchParsimony(name:string, node:Node) {
    ...
    node.children.forEach( (child) => {
        ...
        U = [...U, ...childStates.filter( (state:any) => !U.includes(state) )]; // take the union
        I = (I === undefined ? childStates : childStates.filter( (state:any) => I.includes(state) )); // take the intersection
    });
...
function reconstructInternalStates(name: string, parentStates: any[], acctran: boolean, node: Node  ) {
    ...
    if (node.children) {
        ...
        nodeStates
            .forEach((state:string) => stateCounts[state] = (stateCounts[state] ? stateCounts[state] += 1 : 1) );
...
class Node implements NodeRef{
    ...
    set annotations(value) {
        //TODO check if annotations are valid
    ...
    getClade(tipNameMap:Map<Node,number>){

        let bits:number[]= [];
        if(!this.children){
            const nodeNumericId =  tipNameMap.get(this)!;

            bits = bits.concat(nodeNumericId);
            this._clade = bits;
        }else{
    ...
    toJS():object{
        return ({
                ...
                children: this.children.length>0?this.children.map(child=>child!.toJS()):[],
                });
}
