...
import type { Maybe, Undefinable} from "@figtree/maybe/maybe";
...
import { maybeGetNameFromIndex, maybeGetTaxonByName } from "../Taxa/helperFunctions";
...
export function maybeGetAnnotation(tree:ImmutableTree,node:NodeRef,name:string):Maybe<Annotation>{
        const a = (tree.getNode(node.number) as Node).annotations[name] as Undefinable<Annotation>
        if(a===undefined){
        return Nothing()
        }else{
        return Some(a)
...
export function maybeGetTaxon(tree:ImmutableTree,id:number|NodeRef|string):Maybe<Taxon>{
    if (typeof id === "number") {
          ...
          return maybeGetTaxonByName(tree.taxonSet._data,maybeGetNameFromIndex(tree.taxonSet._data,id))
