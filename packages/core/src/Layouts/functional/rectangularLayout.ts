...
export function rectangularLayout (traversal:PreOrderTraversalCache = new PreOrderTraversalCache()){
    return function layout(tree:Tree,node:NodeRef):Vertex{
        if( traversal.cache.has(node))return traversal.cache.get(node)!
            ...
            if(tree.isExternal(node)){
                if(predecessor){
                    const currentY = layout(tree,predecessor!).maxY; // here recursion happens 
                ...
                const kidPositions = tree.getChildren(node).map(child=>layout(tree,child))
