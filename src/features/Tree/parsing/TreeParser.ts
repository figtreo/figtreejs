...
export function getTreeFromNewick(newick: string, options?: { label: string, datePrefix: string | null, parseAnnotations: boolean }) {
...
export function parseNewick(newick: string, inputOptions?: { label: string, datePrefix: string | null, parseAnnotations: boolean }): TreeState {
    const taxonSet = new TaxonSet();//TODO remove

    const options: { label: string, datePrefix: string | null, parseAnnotations: boolean } = { ...{ label: "label", datePrefix: null, dateFormat: "decimal", tipNameMap: null, parseAnnotations: true }, ...inputOptions }
    ...
    for (const token of tokens) {
        ...
        if (options.parseAnnotations && token.length > 2 && token.substring(0, 2) === '[&') {
            ...
            if (lengthNext) {
                ...
                const taxon = taxonSet.has(name) ? taxonSet.getTaxon(name) : new Taxon(name);
